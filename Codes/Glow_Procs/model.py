# 'model.py' and 'modules.py' were modified to add the following to the Glow implementation on Github:
#  - the full latent vector (concatenation of Zs at all scales) should be returned
#  - the whole latent variable should appear to come from a N(0,I) distribution (we do it by nomalizing the values computed by the current implementation)
...
from modules import (Conv2d, Conv2dZeros, ActNorm2d, InvertibleConv1x1,
                     ...
                     Split2d, gaussian_likelihood, gaussian_sample,ToStandard)
...
class FlowNet(nn.Module):
    ...
    def forward(self, input, logdet=0., reverse=False, temperature=None,return_stan_codes=False):
        if reverse:
            return self.decode(input, temperature)
        else:
            return self.encode(input, logdet,return_stan_codes)
    ...
    def encode(self, z, logdet=0.0, return_stan_codes=False): #*
        # return_stan_codes: if the normalized latent vector should be returned

        codes=[]

        for layer, shape in zip(self.layers, self.output_shapes):
            if not isinstance(layer, Split2d):
                z, logdet = layer(z, logdet, reverse=False)
            else:
                z, logdet, code = layer(z, logdet, reverse=False)
                if return_stan_codes:
                    codes.append(code)

        if not return_stan_codes:
            return z, logdet
        else:
            return z, logdet, codes
...
class Glow(nn.Module):
    ...
    def forward(self, x=None, y_onehot=None, z=None, temperature=None,
                reverse=False,return_latent_mode=None):
        if reverse:
            return self.reverse_flow(z, y_onehot, temperature)
        else:
            return self.normal_flow(x, y_onehot,return_latent_mode)

    def normal_flow(self, x, y_onehot,return_latent_mode='full'): #*
        #return_latent_mode: none, full, onlydeep
        # with 'onlydeep', only the latent dimesnions corresponding to the last step in multiscale architecture are returned
        ...
        if return_latent_mode=="none" or return_latent_mode=="onlydeep":
            z, objective = self.flow(x, logdet=logdet, reverse=False)
        elif return_latent_mode=="full":
            z, objective,codes = self.flow(x, logdet=logdet, reverse=False,return_stan_codes=True)
            latent_stan_codes=codes

        mean, logs = self.prior(x, y_onehot)
        objective += gaussian_likelihood(mean, logs, z)

        if return_latent_mode == "full":
            latent_stan_codes.append(ToStandard(z,mean,logs))
        if return_latent_mode == "onlydeep":
            latent_stan_codes=ToStandard(z,mean,logs)
        ...
        # returning latent code / log-likelihood
        if return_latent_mode == 'none':
            return objective
        else:
            return latent_stan_codes, objective
